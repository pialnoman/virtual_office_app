  ...
  const CreateNewProject = () => {
    ...
    const create_project = (values,{ setSubmitting }) => {
      ...
      const formValues={
          ...
          "pm": values.pm,
          "planned_hours": (values.planned_hours).toFixed(2),
          "planned_value": (values.planned_value).toFixed(2),
          "remaining_hours": (values.remaining_hours).toFixed(2)
      }
    ...
    function handleDeliveryDateExtend(event) {
      ...
      .then((res) => {
        ...
        // call update assignee list function here
        formCreateProject.setValues({
          task_delivery_order: formCreateProject.values.task_delivery_order,
          tdo_details: formCreateProject.values.tdo_details,
          sub_task: formCreateProject.values.sub_task,
    ...
    //  inputList.forEach((item) => removeAssignee(item));
      formCreateProject.handleChange(event); //setting planned delivery date
