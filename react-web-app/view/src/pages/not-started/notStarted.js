...
const NoWbs = () => {
  ...
  React.useEffect(() => {
    API.get("project/assignees-with-no-wbs-rapid/").then((res) => {
      let temp = [];
      console.log("Rain api response", res.data.data);

      for (let i = 0; i < res.data.data.length; i++) {
        // assignees to allassignees
        if (res.data.data[i].allassignees.length != 0  && dateOver(res.data.data[i].project.planned_delivery_date)) {
      ...
      for (let i = 0; i < res.data.data.length; i++) {
        ...
        if(1 === 1){
          ...
          if (res.data.data[i].allassignees.length != 0) {
            tableData.push({
              TDO: res.data.data[i].project.task_delivery_order.title,
              "Sub Task": res.data.data[i].project.sub_task,
              "Work Package Number": res.data.data[i].project.work_package_number,
      ...
      <CRow>
        <CCol lg="9">
          <h3 className="timecards-page-header mb-3">
            ...
            Create WBS for Assigned Projects {" "}
          </h3>
        </CCol>
